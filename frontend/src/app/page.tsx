...
import {
  ...
  BrainCircuit, Globe2, Compass, BarChart3, ShieldCheck
} from 'lucide-react';
...
export default function Page() {
  ...
  // Hydration guard — server has no localStorage, so we start unmounted
  ...
  // Mount effect — rehydrate auth from localStorage (client-only)
  useEffect(() => {
  ...
  // WebSocket lifecycle — only active when authenticated
  useEffect(() => {
    if (!isAuthenticated) {
      if (wsRef.current) {
        wsRef.current.close();
    ...
    const connectWS = () => {
      ...
      ws.onopen = () => console.log('[WS] Connected to India sensor stream.');
      ...
      ws.onclose = () => {
        console.log('[WS] Disconnected — reconnecting in 3.5s...');
  ...
  // Show neutral spinner until client has mounted (prevents hydration mismatch)
  if (!mounted) {
    return (
  ...
  // ─── LOGIN HANDLER (defined after hooks, used only in JSX below) ────────
  ...
  if (!isAuthenticated) {
    return (
      <main className="relative min-h-screen w-full flex items-center justify-center bg-slate-950 text-slate-100 overflow-hidden tech-grid p-4">
        <div className="absolute top-1/4 left-1/4 w-80 h-80 rounded-full bg-cyan-600/10 blur-[100px] pointer-events-none" />
        ...
        <div className="w-full max-w-md glass-panel p-8 rounded-2xl border border-slate-800 shadow-[0_0_50px_rgba(0,0,0,0.8)] relative z-10">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            {loginError && (
              <div className="p-3 text-xs font-mono bg-rose-950/20 border border-rose-800 rounded-lg text-rose-400 text-center">
                {loginError}
              </div>
            )}
            ...
            <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800/80 text-[9px] font-mono text-slate-500 leading-normal">
              ...
              <div>• Command Director: <span className="text-cyan-400">admin</span> / <span className="text-cyan-400">admin123</span></div>
              <div>• Field Operative: <span className="text-cyan-400">field_agent</span> / <span className="text-cyan-400">field123</span></div>
            ...
            <button
              type="submit"
              disabled={loggingIn}
              className="w-full py-3 bg-gradient-to-r from-cyan-600 to-indigo-600 hover:from-cyan-500 hover:to-indigo-500 text-white font-mono text-xs uppercase tracking-widest font-semibold rounded-lg shadow-[0_0_15px_rgba(6,182,212,0.25)] transition-all cursor-pointer disabled:opacity-60"
            >
            ...
            <div className="px-4">
              ...
              <div className="space-y-1.5 font-mono text-[9px] text-slate-400">
                <div className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse neon-pulse-dot" />
                ...
                <div className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-cyan-500 animate-pulse neon-pulse-dot" />
